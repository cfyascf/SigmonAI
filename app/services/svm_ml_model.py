...
class SVMMLModel(BaseModel):
    __path = fr'{os.path.dirname(os.path.abspath(__file__))}\models\ml_svm.pkl'
    __categories = fr'{os.path.dirname(os.path.abspath(__file__))}\models\ml_svm_label_encoder.pkl'
    __instance = None
    ...
    @staticmethod
    def get_instance():
        if(SVMMLModel.__instance is None):
            svm = SVMMLModel()
            SVMMLModel.__instance = svm

        return SVMMLModel.__instance
