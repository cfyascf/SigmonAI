...
class NNAndreModel(BaseModel):
    __path = fr'{os.path.dirname(os.path.abspath(__file__))}\models\andre_nn.sav'
    ...
    @staticmethod
    def get_instance():
        if(NNAndreModel.__instance is None):
            nn = NNAndreModel()
            NNAndreModel.__instance = nn

        return NNAndreModel.__instance
